...
public class ReviewHandler extends AbstractHandler {
    // host/api/review/{course_id}/{review_id}
    @Override
    protected String fetch(String[] pathParts, String query) {
        ...
        if (query != null && query.contains("user_id") && query.contains("course_id")) {
            ...
            response = db.fetchReviews(Integer.parseInt(pathParts[3]));
        } else if (pathParts.length == 5) {
            response = db.fetchReview(Integer.parseInt(pathParts[3]), pathParts[4]);
        } else {
